...
import json,os
r=json.loads(Path('review-evidence/after/audit.json').read_text())
i=json.loads(Path('review-evidence/interaction-checks.json').read_text())
h=json.loads(Path('docs/historical-image-manifest.json').read_text())
assert not r['failures'] and all(x['pass'] for x in i['checks'])
run=os.environ.get('GITHUB_RUN_ID','local')
report=f'''# StartupFair template and database review
...
- {len(r['viewports'])} browser page/viewport checks at widths 1440, 1024, 768 and 390 pixels.
- {len(r['routes'])} distinct internal routes checked for HTTP success and meaningful page headings, including form steps, policy and confirmation routes.
- {len(r['modal'])} popup viewport checks: contained white panel, working field entry, recipient choice and Escape close.
- {len(i['checks'])} actual browser navigation, inquiry dropdown and close-button checks across desktop and mobile.
...
Verification run: https://github.com/pks-alt/startupfair-site/actions/runs/{run}
...
'''
Path('docs/SITE-REVIEW-RESULTS.md').write_text(report)
Path('review-evidence/SITE-REVIEW-RESULTS.md').write_text(report)
