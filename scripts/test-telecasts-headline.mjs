...
const report={testedApplicationSource:'1fe5fc936c0c51bb602a8809eb4a7aac83eaa72c plus requested video/headline changes',headlines:[],videos:[],popup:[],youtubeMetadata:[],playbackVerified:false};
async function open(page,path){await page.goto(base+path,{waitUntil:'domcontentloaded'});await page.locator('h1').first().waitFor();await page.evaluate(()=>document.fonts.ready);await page.waitForFunction(()=>[...document.images].every(i=>i.complete),{},{timeout:10000}).catch(()=>{});}
try{
...
}finally{writeFileSync(`${out}/video-headline-verification.json`,JSON.stringify(report,null,2));await browser.close();}
